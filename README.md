...
> This is a fork of [nielsfaber/alarmo-card](https://github.com/nielsfaber/alarmo-card) that adds sound support (`pending_sound`, `click_sound`, `disarm_sound`) and a `hide_arm_options` toggle. Everything else follows upstream. See [Sounds](#sounds).
...
### Sounds
The card can play sounds, which is useful on a wall-mounted tablet where a touch gives no tactile feedback. Each sound is a url to an audio file, set in the card editor or in YAML, and each is off until you set one:

| Option          | Plays                                                                    |
| --------------- | ------------------------------------------------------------------------ |
| `pending_sound` | On a loop while the alarm is arming or pending, as a countdown cue.       |
| `click_sound`   | Once per press, on the keypad and the arm/disarm buttons.                 |
| `disarm_sound`  | Once when the alarm is disarmed, confirming the code was accepted.        |

```yaml
type: custom:alarmo-card
entity: alarm_control_panel.alarmo
pending_sound: /local/alarm_arming.mp3
click_sound: /local/click.mp3
disarm_sound: /local/disarmed.mp3
```

Place the files in the `www` folder of your Home Assistant configuration, and they are served from `/local/`. A new `www` folder is only picked up on restart.

While the alarm counts down, the pending sound steps aside briefly on each button press so the click stays audible, then resumes once you stop typing.

**Use `.mp3`.** The browser has to be able to decode the file, and `.mp3` is the safest choice. Home Assistant serves `.wav` as `audio/vnd.wave`, which Firefox refuses to play. If a sound stays silent, open its url directly in the browser — if it will not play there, it will not play in the card.

Browsers also block audio until the page has been interacted with, so the very first sound after loading a dashboard may not play.
...
| `pending_sound`         | string  | Optional     | Url to a sound clip, played on a loop while the alarm is arming or pending.<br>See [sounds](#sounds).                                                                            |                    |
| `click_sound`           | string  | Optional     | Url to a sound clip, played once when a keypad or arm/disarm button is pressed.<br>See [sounds](#sounds).                                                                        |                    |
| `disarm_sound`          | string  | Optional     | Url to a sound clip, played once when the alarm is disarmed.<br>See [sounds](#sounds).                                                                                           |                    |
